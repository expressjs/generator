...
function createApplication (name, dir, options, done) {
  ...
  var app = {}
  // JavaScript
  exec('node -v', function (errorval, stdout, stderr) {
    if (errorval !== null) {
      console.log('Error : ' + errorval)
    }
    if (stdout) {
      var nodeVersion = parseFloat(stdout.split('v')[1])
      if (nodeVersion >= 20) {
        app = loadTemplate('js/es6appandwww/app.js')
        www = loadTemplate('js/es6appandwww/www')
      } else {
    ...
    if (nodeVersion >= 20) {
      copyTemplateMulti('js/es6routes', dir + '/routes', '*.js')
    } else {
